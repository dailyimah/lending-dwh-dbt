...
*/
with f as (
    select * from "fazz_dwh"."main_staging"."stg_fund"
),
fr as (
    select fund_id,
    ...
    from "fazz_dwh"."main_staging"."stg_fund_record"
    group by fund_id
),
lender_asof as (
...
lender_earliest as (
    ...
    from f
    join "fazz_dwh"."main_warehouse"."dim_customer" d on d.customer_id = f.lender_customer_id
),
fl as (
...
select
    ...
    coalesce(la.customer_key, le.customer_key, 'UNKNOWN#1') as lender_customer_key,
    ...
    case when fl.nominal_loan > 0
         then cast(f.committed_amount as float) / cast(fl.nominal_loan as float) end as share_of_loan,
    ...
    coalesce(fr.settled_amount, 0) >= f.committed_amount - 1 as is_fully_settled,
