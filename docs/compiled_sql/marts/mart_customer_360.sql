...
*/
with cur as (
    select * from "fazz_dwh"."main_warehouse"."dim_customer" where is_current and customer_id <> 'UNKNOWN'
),
loans as (
    select
...
snap as (
    select
        ...
        max(case when s.snapshot_date = cast('2026-08-21' as date) and s.is_active then s.days_past_due else 0 end) as current_dpd,
...
sched as (
    ...
    from "fazz_dwh"."main_warehouse"."fact_repayment_schedule" s
    join "fazz_dwh"."main_warehouse"."fact_loan" l on l.loan_id = s.loan_id
    where s.due_date <= cast('2026-08-21' as date)
    group by l.borrower_customer_id
),
funding as (
    select
    ...
    from "fazz_dwh"."main_warehouse"."fact_funding"
...
select
    ...
    case
  when coalesce(sn.current_dpd, 0) is null or coalesce(sn.current_dpd, 0) < 1 then 'current'
  when coalesce(sn.current_dpd, 0) < 31 then '1-30'
  when coalesce(sn.current_dpd, 0) < 61 then '31-60'
  when coalesce(sn.current_dpd, 0) < 91 then '61-90'
  else '90+'
    ...
    case when sc.installments_due_count > 0
         then cast(sc.installments_paid_on_time_count as float) / sc.installments_due_count end as on_time_repayment_rate,
    sc.avg_days_late,
